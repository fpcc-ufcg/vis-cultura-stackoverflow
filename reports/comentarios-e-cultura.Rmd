...
No processo de melhora das visualizações, podemos agora analisar a dispersão entre os pares de variáveis, podendo observar a existência de padrões e obtendo precisão na localização dos pontos. 
...
data <- dados %>% select(comentaram_prop, PDI, Internet, usuarios, six_regions)

data %>%
    select(comentaram_prop, PDI, Internet, usuarios) %>%
    pairs(pch = 19,  cex = 0.5,
      col = my_cols[data$six_regions],
      lower.panel=NULL)
```
